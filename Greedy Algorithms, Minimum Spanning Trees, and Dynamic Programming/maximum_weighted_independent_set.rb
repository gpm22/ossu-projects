class Graph
  ...
  def getMaximumWeightedIndependentSetValue
    ...
    (2..@nodes.size).each do |i|
      @subproblemsSolutions[i] =
        [@subproblemsSolutions[i - 1], (@subproblemsSolutions[i - 2] + @nodes[i - 1].weight)].max
    end
  ...
  def reconstructIndependentSet
    ...
    while i >= 2
      if @subproblemsSolutions[i - 1] >= @subproblemsSolutions[i - 2] + @nodes[i - 1].weight # case 1 wins
        i -= 1 # exclude node[i-1]
      else # case 2 wins
        independentSet.push(@nodes[i - 1]) # include node[i-1]
        i -= 2 # exclude node[i-2]
      end
    end

    independentSet.push(nodes[0]) if i == 1 # base case 2
...
class Node
  ...
  def to_s
    "Node:[key: #{@key}, weight: #{@weight}, neighbors: #{@neighbors.map(&:key)}]"
  end
...
def testGetMWIS
  ...
  puts("#{expected == real ? 'approved' : "wrong: real #{real.map(&:to_s)} and not #{expected.map(&:to_s)}"}")
end
