...
def test(arr, t, result)
    puts "hash test - arr=#{arr.size} - t: #{t.to_s} - expected result: #{result} - passed? #{result == two_sum_hash(arr, t)? "yes!" : "non ):"}"
    puts "set test - arr=#{arr.size} - t: #{t.to_s} - expected result: #{result} - passed? #{result == two_sum_set(arr, t)? "yes!" : "non ):"}"
end
