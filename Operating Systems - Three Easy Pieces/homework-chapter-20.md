...
1. **With a linear page table, you need a single register to locate the page table, assuming that hardware does the lookup upon a TLB miss. How many registers do you need to locate a two-level page table? A three-level table?**
   Still needing just **one base register**, that will point to the first-level table, so you can get the page directory or go to the next table.

2. **Use the simulator to perform translations given random seeds 0, 1, and 2, and check your answers using the -c flag.**

   The virtual address uses 15 bits, 10 for the VPN.
   The upper 5 bits of a virtual address are used to index into a page directory.
   ...
   page  53:0f 0c 18 09 0e 12 1c 0f 08 17 13 07 1c 1e 19 1b 09 16 1b 15 0e 03 0d 12 1c 1d 0e 1a 08 181100
   ...
   page  78:0e 02 17 1b 1c 1a 1b 1c 10 0c 15 08 19 1a 1b 12 1d 11 0d 14 1e 1c1802120f131a07160306
   ...
   page  86:7f 7f 7f 7f 7f 7f 7f c5 7f 7f 7f 7f 7f 7f 7f7f7f7f7f7fca7f7fee7f7f7f7f7f7f7f7f
   ...
   page 108:83 fe e0 da 7f d4 7f eb be 9e d5 ad e4 ac 90 d6 92 d8 c1 f8 9f e1 ed e9 a1e8c7c2a9d1dbff
   ...
   page 127:7f 7f 7f 7f 7f 7f 7f 7f 7f 7f 7f 7f 7f 7f 7f 7f df 7f 7f 7f 7f 7f 7f 7f 7f 7f 7f 7f 7f957f7f
   ...
   # 0x611c -> 11000 01000 11100, so the PDIndex is 0x18 -> 24, which contains 0xa1
   # 0xa1 -> 10100001, which is valid, pfn 0x21 -> page 33
   # The pte is 0x8 -> 8, which contains 0xb5
   # 0xb5 1 0110101, which is valid, pfn is 0x35 > 53
   # It translates to 0110101 11100, which is 0x6BC
   # So the in page 53 with offset 28 is 0x08
   
   Virtual Address 3da8: Translates To What Physical Address (And Fetches what Value)? Or Fault?
   
   # 3da8 -> 01111 01101 01000, so the PDINDEX is 15, which contains 0xd6
   # 0xd6 -> 1 1010110, which is valid, pfn -> 86
   # the pte is 13, which contains 0x7f
   # 0x7f -> 0 1111111, which is invalid
   # FAULT
   
   Virtual Address 17f5: Translates To What Physical Address (And Fetches what Value)? Or Fault?
   
   # 17f5 -> 00101 11111 10101, so the PDIndex is 5, which contains 0xd4
   # 0xd4 -> 1 1010100, which is valid, pfn -> 84
   # the pte is 31, which contains 0xce
   # 0xce -> 1 1001110, which is valid, pfn -> 1001110
   # physical address is 1001110 10101, which is 0x9D5
   # which contains 0x1c in the page 78, offset 21 
   
   Virtual Address 7f6c: Translates To What Physical Address (And Fetches what Value)? Or Fault?
   
   # 7f6c -> 11111 11011 01100, so the PDINDEX is 31, which contains 0xff
   # 0xff -> 1 1111111, which is valid, pfn -> 127
   # the pte is 27, which contains 7f
   # 0x7f -> 0111 1111, which is invalid
   # FAULT
   
   Virtual Address 0bad: Translates To What Physical Address (And Fetches what Value)? Or Fault?
   
   # got tired, this is repetitive and obvious
   ...
   **How many memory references are needed to perform each lookup?**
   ...
   1 to get the PDE, 1 to get the PTE, 1 to get the value itself.
   Thus 3.
...
3. **Given your understanding of how cache memory works, how do you think memory references to the page table will behave in the cache? Will they lead to lots of cache hits (and thus fast accesses?) Or lots of misses (and thus slow accesses)?**
   When a TLB hit happens, the first level is ignored. So if the TLB hits are frequent, without using the first level, the access to the page table will be slow with a lot of misses, because it will not be used frequently.
   It can be even worse if the control flow is the same as shown in the table **Multi-level Page Table Control Flow**:
   ...
   Because the PDE address is never even inserted to the TLB, so they would be all TLB MISSES!
